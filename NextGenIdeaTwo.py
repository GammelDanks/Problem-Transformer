...
def fetch_from_google(query, num_results=10, trusted_only=True):
    ...
    # Primary search query (trusted sources only)
    trusted_query = (
    ...
    # If trusted_only is False, perform a general search without site restrictions
    search_query = trusted_query if trusted_only else (
        f'{query} ("report" OR "study" OR "data analysis" OR "white paper") '
        f'-site:pinterest.com -site:quora.com -site:reddit.com after:{current_year - 3}'
    )

    url = f"https://www.googleapis.com/customsearch/v1?q={search_query}&key={GOOGLE_API_KEY}&cx={SEARCH_ENGINE_ID}&num={num_results}"
