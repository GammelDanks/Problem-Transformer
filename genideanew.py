...
# Step 2: Retrieve existing solutions
def retrieve_existing_solutions(problem_description):
    prompt = f"""
    Provide a summary of real-world solutions, scientific research, and patents related to:
    {problem_description}.
    Focus on relevant technologies and industries where similar solutions have been implemented.
    """

    response = openai.ChatCompletion.create(
        model="gpt-4-turbo",
        messages=[{"role": "system", "content": "You are an AI researcher gathering relevant technologies and patents."},
                  {"role": "user", "content": prompt}]
    )
...
# Step 3: Generate tech-driven ideas
...
# Step 4: Evaluate & refine best idea
...
if st.button("Generate Solutions"):
    if problem_description and target_audience:
        try:
            ...
            with st.spinner("Retrieving existing solutions..."):
                existing_solutions = retrieve_existing_solutions(problem_description)
            st.subheader("Existing Solutions & Research")
            st.write(existing_solutions)

            with st.spinner("Generating new innovative ideas..."):
                new_ideas = generate_new_ideas(problem_description, target_audience, existing_solutions)
